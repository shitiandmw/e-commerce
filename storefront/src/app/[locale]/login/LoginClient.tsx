  ...
  return (
    <AuthLayout>
      <div className="rounded-lg border border-border bg-surface p-8">
        <h1 className="mb-6 text-center text-2xl font-bold text-foreground">登录</h1>
        {serverError && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end">
            <Link href={`/${locale}/forgot-password`} className="text-sm text-gold hover:text-gold-light">忘记密码？</Link>
          </div>
        ...
        <p className="mt-4 text-center text-sm text-muted">
          还没有账户？{" "}
          <Link href={`/${locale}/register`} className="text-gold hover:text-gold-light">立即注册</Link>
        </p>
      </div>
    </AuthLayout>
  )
