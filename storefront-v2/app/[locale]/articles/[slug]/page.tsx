...
export default async function ArticleDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const { articles: allArticles } = await fetchArticles({ limit: 50, locale })
  ...
  return (
    <div>
      {/* Breadcrumb */}
      <nav className="mx-auto max-w-4xl px-4 py-4 lg:px-6" aria-label="breadcrumb">
        <ol className="flex items-center gap-1.5 text-xs text-muted-foreground">
          <li><Link href="/" className="hover:text-gold transition-colors">{t("home")}</Link></li>
          <li><ChevronRight className="size-3" /></li>
          <li><Link href="/articles" className="hover:text-gold transition-colors">{t("breadcrumb_articles")}</Link></li>
          <li><ChevronRight className="size-3" /></li>
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 lg:px-6">
        ...
        <div className="mt-12 pt-8 border-t border-border/30">
          <p className="text-sm text-muted-foreground mb-3">{t("share_article")}</p>
        ...
        {relatedArticles.length > 0 && (
          <div className="mt-16">
            <h2 className="text-xl font-serif font-bold text-foreground mb-8">{t("related_articles")}</h2>
