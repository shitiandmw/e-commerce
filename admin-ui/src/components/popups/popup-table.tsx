...
import { usePopups, Popup, useDeletePopup } from "@/hooks/use-popups"
import { getPopupColumns } from "./popup-columns"
...
export function PopupTable() {
  ...
  const { data, isLoading, isError, error } = usePopups({
    offset: pagination.pageIndex * pagination.pageSize,
    limit: pagination.pageSize,
  })

  const deletePopup = useDeletePopup()
  const [popupToDelete, setPopupToDelete] = React.useState<Popup | null>(null)

  const handleDeleteConfirm = async () => {
    if (!popupToDelete) return
    try {
      await deletePopup.mutateAsync(popupToDelete.id)
  ...
  const columns = React.useMemo(
    () => getPopupColumns((popup) => setPopupToDelete(popup), t),
    [t]
  )
